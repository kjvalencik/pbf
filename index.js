...
Pbf.prototype = {
    ...
    readVarint: function(isUnsigned) {
        var buf = this.buf,
            val, b;
        ...
        return readVarintRemainder(val, isUnsigned, this);
    },
    ...
    readPackedVarint: function(type, arr, isUnsigned) {
        var end = type === Pbf.Bytes ? this.readVarint() + this.pos : this.pos + 1;
        while (this.pos < end) arr.push(this.readVarint(isUnsigned));
        return arr;
    },
...
function readVarintRemainder(l, u, p) {
    ...
    b = buf[p.pos++]; h  = (b & 0x70) >> 4;  if (b < 0x80) return toNum(l, h, u);
    b = buf[p.pos++]; h |= (b & 0x7f) << 3;  if (b < 0x80) return toNum(l, h, u);
    b = buf[p.pos++]; h |= (b & 0x7f) << 10; if (b < 0x80) return toNum(l, h, u);
    b = buf[p.pos++]; h |= (b & 0x7f) << 17; if (b < 0x80) return toNum(l, h, u);
    b = buf[p.pos++]; h |= (b & 0x7f) << 24; if (b < 0x80) return toNum(l, h, u);
    b = buf[p.pos++]; h |= (b & 0x01) << 31; if (b < 0x80) return toNum(l, h, u);

    throw new Error('Expected varint not more than 10 bytes');
}

function toNum(low, high, isUnsigned) {
    if (isUnsigned) {
